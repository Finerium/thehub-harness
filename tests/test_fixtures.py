...
TOP_KEYS = {"chains", "coverage", "coverage_bands", "coverage_by_tag", "coverage_by_work_type", "coverage_scores",
            ...
            "workbook", "coverage_labels", "demo", "population_ids"}  # the last three: 10.5 registry keys
...
def test_workbook_totals():
    ...
    assert w["breakdown_kinds"] == {"planned_flagged": {"cost_idr": 69_270_000, "count": 8, "downtime_h": 164.0, "hours": 164.0, "rows": 8},
                                    "unplanned": {"cost_idr": 344_075_000, "count": 23, "downtime_h": 270.0, "hours": 270.0, "rows": 23}}  # 10.5 rows and hours beside count and downtime_h
    assert w["breakdown_flagged"] == {"rows": 31, "hours": 434.0, "cost_idr": 413_345_000}
...
def test_populations():
    p = FX["population_ids"]  # the sorted work-order numbers; 10.5 populations.<name> carries the counts
    assert FX["populations"] == {k: len(v) for k, v in p.items()} == {"all": 211, "failure": 66, "unplanned_failure": 57, "planned_flagged": 8, "unplanned_breakdowns": 23}
...
def test_coverage_rows_exist_for_both_layers_and_every_population():
    assert set(FX["coverage"]) == {"generous", "strict", "bands"}  # 10.5 coverage.bands sits beside the two layers
    for layer in ("generous", "strict"):
        assert set(FX["coverage"][layer]) == set(FX["populations"]) == set(FX["population_ids"])
        for pop in FX["populations"]:
            r = row(layer, pop)
            assert r["n"] == FX["populations"][pop] and r["uncovered"] == len(r["uncovered_ids"]) and set(r["uncovered_ids"]) <= set(FX["population_ids"][pop])
...
def test_bands_sum_to_57():
    ...
    assert sorted(b["none_ids"] + b["table_only_ids"] + b["taught_ids"]) == FX["population_ids"]["unplanned_failure"]
    assert FX["coverage"]["bands"]["unplanned_failure"] == [{"t": T, "no_lesson": 14, "copied_row_only": 27, "taught": 16}]  # the 10.5 twin
...
def test_integrity_total_is_the_sum_of_its_defect_rules():
    ...
    assert i["rules"] == {k: v["count"] for k, v in rules.items() if not v["observation_only"]}  # the 10.5 twins
    assert i["observations"] == {k: v["count"] for k, v in rules.items() if v["observation_only"]}
...
def test_debt_ranking():
    d = FX["debt"]
    assert d["coefficients"] == {"a": 0.4, "b": 0.3, "c": 0.2, "d": 0.1, "basis": "ASSUMPTION"}  # the 9.5 DebtCluster shape
    assert d["coefficients_by_factor"] == {"C": 0.3, "D": 0.4, "k": 0.2, "r": 0.1, "basis": "product decision, ASSUMPTION"}
    ...
    assert all(p["k"] == d["k_mapping"][crit[p["tag"]]] and p["r"] == FX["families"]["r_by_tag"][p["tag"]] == FX["families"]["r_detail"][p["tag"]]["r"] for p in d["per_asset"])
    ...
    again = debt.rank(workbook.load(), row("generous", "unplanned_failure")["uncovered_ids"], crit, dict(FX["families"]["r_by_tag"]))
    assert ([{k: p[k] for k in a} for a, p in zip(again["per_asset"], d["per_asset"])], again["ranking"]) == (again["per_asset"], d["ranking"])
    assert again["coefficients"] == d["coefficients_by_factor"]
    # the 10.5 twins beside the older keys carry the same values
    assert all(p["D_h"] == p["D"] and p["C_idr"] == p["C"] and p["uncovered_wo_numbers"] == p["uncovered_ids"] for p in d["per_asset"])
    assert {p["tag"]: p["rank"] for p in d["per_asset"]} == {r["tag"]: r["rank"] for r in d["ranking"]}
...
def test_cross_module_reconciliations():
    ...
    assert FX["method"]["labels_status"] == "machine_drafted_pending_human" == FX["coverage_labels"]["status"]  # the 9.5 enum
    assert FX["method"]["labels_note"].startswith("machine-drafted labels present")  # the draft is evidence, not a deck number
...
def test_fixture_validates_against_the_registry_schema():
    ...
    v = jsonschema.Draft202012Validator(schemas["fixtures.schema.json"], registry=registry(schemas))
...
POPULATIONS = ["unplanned_failure", "failure", "unplanned_breakdowns", "planned_flagged", "all"]
CELL = ["uncovered", "share", "breakdowns", "downtime_h", "cost_idr"]
REGISTRY_PATHS = (
    ["inventory.files_total", "inventory.by_class", "inventory.corpus_sha256", "inventory.extractor", "inventory.canonical_form_version"]
    + ["method." + k for k in ("recipe_sha256", "stop_list_sha256", "threshold", "window_multiplier", "min_content_words", "strict_rule",
                               "strict_sections", "strict_cut_marker", "labels_status", "unscoreable_ids")]
    + ["workbook." + k for k in ("rows", "reported_first", "reported_last", "completed_last", "work_types", "all_cost_idr", "all_labor_hours",
                                 "discipline_counts")]
    + ["workbook.breakdown_flagged." + k for k in ("rows", "hours", "cost_idr")]
    + [f"workbook.breakdown_kinds.{kind}.{k}" for kind in ("unplanned", "planned_flagged") for k in ("rows", "hours", "cost_idr")]
    + ["workbook.incomplete." + k for k in ("rows", "share", "by_work_type", "emergency_rows")]
    + ["workbook.lead_time." + k for k in ("median_h", "at_least_24h", "share", "min_h", "max_h")]
    + ["populations." + p for p in POPULATIONS]
    + ["coverage_labels." + k for k in ("status", "uncovered_ids", "uncovered_count", "share", "breakdowns", "downtime_h", "cost_idr",
                                        "kappa_covered", "kappa_taught")]
    + [f"coverage_labels.proxy_agreement.{layer}.{k}" for layer in ("generous", "strict") for k in ("precision", "recall", "fp", "fn")]
    + ["verbatim." + k for k in ("problem_description", "root_cause", "corrective_action", "any_field", "strict_any_field")]
    + ["dates." + k for k in ("parsed", "first", "last", "window_days", "median_gap_d", "mode_gap_d", "mode_count", "within_set_gap_d",
                              "days_after_last_breakdown")]
    + ["latency.pairs", "latency.median_d"]
    + ["integrity.total", "integrity.rules", "integrity.observations.CD-15", "integrity.observations.CD-16"]
    ...
    + ["proof_tests.total"] + ["proof_tests.by_class." + k for k in ("sis_proof_test", "sil_logic_test", "calibration_proof_test", "statutory_relief_test")]
    + ["equipment_master"]
    + ["personnel." + k for k in ("people", "classes", "dual_role_supervisors", "managers_not_in_workbook")]
...
def test_every_10_5_key_path_exists():
    ...
    for layer in ("generous", "strict"):
        for pop in POPULATIONS:
            ...
            missing += [f"coverage.{layer}.{pop}[{r['t']}].{k}" for r in rows for k in CELL if k not in r]
    bands = {r["t"]: r for r in FX["coverage"]["bands"]["unplanned_failure"]}
    missing += [f"coverage.bands.unplanned_failure[0.62].{k}" for k in ("no_lesson", "copied_row_only", "taught") if k not in bands.get(0.62, {})]
    assert set(FX["coverage_scores"]) == set(FX["population_ids"]["all"])
    for wo, s in FX["coverage_scores"].items():
        for layer in ("generous", "strict"):
            missing += [f"coverage_scores[{wo}].{layer}.{k}" for k in ("best_ratio", "matched_field", "matched_lesson") if k not in s[layer]]
    rules = {f"CD-{n}" for n in range(1, 19)} - {"CD-3", "CD-15", "CD-16"}  # CD-15 and CD-16 are observations
    missing += [f"integrity.rules.{k}" for k in sorted(rules) if k not in FX["integrity"]["rules"]]
    for pair in FX["latency"]["pairs"]:
        missing += [f"latency.pairs[].{k}" for k in ("wo_number", "tag", "opl_id", "report_date", "date_of_sharing", "days") if k not in pair]
    for p in FX["debt"]["per_asset"]:
        missing += [f"debt.per_asset[{p.get('tag')}].{k}" for k in ("tag", "rank", "uncovered_wo_numbers", "D_h", "C_idr", "k", "r",
                                                                    "incomplete_uncovered", "score") if k not in p]
    for e in FX["equipment_master"]:
        missing += [f"equipment_master[{e.get('tag')}].{k}" for k in ("tag", "service", "interlock_ref", "sil_sheet", "criticality", "work_orders",
                                                                      "failure_rows", "breakdown_rows", "planned_rows", "unplanned_rows",
                                                                      "unplanned_h", "flagged_h", "breakdown_cost_idr") if k not in e]
    assert not missing, missing
    assert FX["debt"]["coefficients"] == {"a": 0.4, "b": 0.3, "c": 0.2, "d": 0.1, "basis": "ASSUMPTION"}
    assert all(t in FX["families"]["r_by_tag"] for t in TAGS) and all(isinstance(v, float) for v in FX["families"]["r_by_tag"].values())
    assert set(FX["integrity"]["rules"]) == rules and sum(FX["integrity"]["rules"].values()) == FX["integrity"]["total"]
...
def test_10_5_stated_values():
    ...
    assert FX["inventory"]["files_total"] == 98 and FX["inventory"]["canonical_form_version"] == "1"
    assert (FX["method"]["threshold"], FX["method"]["window_multiplier"], FX["method"]["min_content_words"]) == (0.62, 2, 3)
    assert FX["method"]["strict_sections"] == ["header", "1", "2", "3", "4", "6"]
    assert FX["method"]["strict_cut_marker"] == "This is sample data provided for CALIBER purposes only"
    assert FX["method"]["labels_status"] in ("machine_drafted_pending_human", "human_adjudicated")
    assert FX["workbook"]["rows"] == 211
    assert FX["populations"] == {"unplanned_failure": 57, "failure": 66, "unplanned_breakdowns": 23, "planned_flagged": 8, "all": 211}
    assert FX["verbatim"]["strict_any_field"] == 0
    assert (FX["dates"]["parsed"], FX["dates"]["window_days"], FX["dates"]["within_set_gap_d"], FX["dates"]["days_after_last_breakdown"]) == (56, 101, 4, 114)
    ...
    assert FX["proof_tests"]["total"] == 33 and FX["proof_tests"]["by_class"] == {"sis_proof_test": 18, "sil_logic_test": 5, "calibration_proof_test": 7, "statutory_relief_test": 3}
    assert (FX["personnel"]["people"], FX["personnel"]["dual_role_supervisors"], FX["personnel"]["managers_not_in_workbook"]) == (23, 3, 2)
    assert (FX["golden"]["size"], FX["golden"]["hard_gate_count"]) == (102, 16)
    assert (FX["legacy"]["headline_all"], FX["legacy"]["note"]) == (158, "evidence only")
    assert FX["demo"] == {"primary_wo": "WO-240007", "backup_wo": "WO-240039", "contrast_wo": "WO-240060"}
