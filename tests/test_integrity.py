...
def test_shape_counts_and_total():
    ...
    for k in counts:
        ...
        assert R[k]["basis"] in ("H", "M", "H+M") and R[k]["severity"] in (
            "high",
            "medium",
            "low",
        )
    ...
    assert (
        R["total"]
        == sum(v for k, v in counts.items() if k not in ("CD-15", "CD-16"))
        == 174
    )
    assert {k: R[k]["basis"] for k in ("CD-2", "CD-7", "CD-8", "CD-9", "CD-18")} == {
        "CD-2": "M",
        "CD-7": "H+M",
        "CD-8": "H+M",
        "CD-9": "M",
...
def test_cd1_cd5_cross_references():
    ...
    assert [it["opl_id"] for it in R["CD-5"]["items"]] == [
        "OPL-GA-1201A-01",
        "OPL-GA-1201A-02",
        "OPL-GA-1201A-03",
        "OPL-GA-1201A-05",
...
def test_cd4_closeouts_and_emergency():
    ...
    assert (
        c["emergency"] == ["WO-240002", "WO-240032", "WO-240083"]
        and R["cd4_emergency"] == 3
    )  # A1
    for it in c["items"]:
        w = BY_WO[it["wo"]]
        assert it["priority"] == w["Priority"] and w["Status"] == "Completed"
        assert all(w[f] for f in W.NARR) and all(
            w[f] in (None, "") for f in W.OUTCOME_FIELDS
        )


def test_cd6_planned_flagged():
    c = R["CD-6"]
    assert [it["wo"] for it in c["items"]] == [
        "WO-240060",
        "WO-240065",
        "WO-240116",
        "WO-240117",
...
def test_cd10_criticality_review():
    ...
    assert [
        (it["tag"], it["criticality"], it["unplanned_h"], it["events"])
        for it in c["items"]
    ] == [("CT-7801", "NON CRITICAL", 46.0, 3), ("EA-5601", "LOW CRITICAL", 54.0, 3)]
    assert {t: v["unplanned_h"] for t, v in c["per_asset"].items()} == {
        "CT-7801": 46.0,
        "DC-3401A": 24.0,
        "EA-5601": 54.0,
        "FA-8901": 12.0,
...
def test_cd11_ga_drawings():
    assert R["CD-11"]["items"] == [
        {"tag": "CT-7801", "evidence": ["DRY WEIGHT past"]},
        {
            "tag": "EA-5601",
            "evidence": [
                "drum vocabulary WATER BOOT / MIST PAD / HORIZONTAL DRUM absent from the datasheet"
            ],
        },
    ]
...
def test_cd12_truncated_cells():
    ...
    assert c["by_lesson"] == {
        **{f"OPL-GA-1201A-0{i}": 4 for i in range(1, 7)},
        **{f"OPL-YD-2301-0{i}": 1 for i in range(1, 8)},
        **{f"OPL-DC-3401A-0{i}": 1 for i in range(1, 8)},
    }
    for it in c["items"]:
        assert (
            it["cell"] != it["field_text"]
            and it["field_text"].startswith(it["cell"])
            and 64 <= len(it["cell"]) <= 69
        )
        assert P.canonical(str(BY_WO[it["wo"]][it["field"]])) == it["field_text"]
    ga1 = {
        (it["wo"], it["field"])
        for it in c["items"]
        if it["opl_id"] == "OPL-GA-1201A-01"
    }
    assert ga1 == {
        ("WO-240002", "Problem_Description"),
        ("WO-240002", "Root_Cause"),
        ("WO-240002", "Corrective_Action"),
        ("WO-240003", "Corrective_Action"),
    }
    dc = {
        it["opl_id"]: (it["wo"], it["cell"][-16:])
        for it in c["items"]
        if it["tag"] == "DC-3401A"
    }
    assert dc["OPL-DC-3401A-01"] == ("WO-240056", "verified reading") and dc[
        "OPL-DC-3401A-07"
    ] == ("WO-240061", "3.2 m3 fresh cat")
    assert all(
        it["cell"].endswith("re-nipped after")
        for it in c["items"]
        if it["tag"] == "YD-2301"
    )
...
def test_cd13_hazard_limits():
    ...
    assert miss["GA-1201A"] == [
        ["16", "barg", "pressure"],
        ["80", "degC", "temperature"],
    ]
    assert miss["KC-4501"] == [
        ["150", "degC", "temperature"],
        ["16", "barg", "pressure"],
    ]
    assert (
        miss["YD-2301"] == [["6", "barg", "pressure"]]
        and miss["LV-6701"] == [["120", "degC", "temperature"]]
        and miss["CT-7801"] == [["50", "degC", "temperature"]]
    )
    assert I.typed_values(
        "DESIGN P/T 16 barg / 200 degC OPERATING TEMP. 95-110 degC 2 kg/cm2g 0.35 bar"
    ) == {
        (16.0, "pressure"),
        (200.0, "temperature"),
        (95.0, "temperature"),
...
def test_cd14_cd15_workbook_observations():
    ...
    assert (
        all(abs(it["diff_idr"]) == 1000 for it in c["items"])
        and c["max_abs_diff_idr"] == 1000
        and not c["observation_only"]
    )
    o = R["CD-15"]
    assert (
        o["count"] == 123
        and o["observation_only"]
        and o["non_chronological_pairs"] == 103
    )
    assert all(
        it["notification_no"].startswith("NT-2024-") and it["report_year"] == 2025
        for it in o["items"]
    )
...
def test_cd16_area_aliases():
    ...
    assert (
        c["observation_only"]
        and [it["tag"] for it in c["items"]] == sorted(c["aliases"])
        and len(c["aliases"]) == 8
    )
    assert all(it["n_vocabularies"] == 2 for it in c["items"])
    assert c["aliases"]["CT-7801"] == {
        "workbook": "COOLING WATER SYSTEM",
        "datasheet": "7800 - COOLING WATER SYSTEM",
        "plot_plan_title": "7800-COOLINGTOWERSYSTEM",
        "opl": "7800 - COOLING TOWER SYSTEM",
    }
...
def test_cd17_and_no_sif_fixture():
    assert R["CD-17"]["items"] == [
        {
            "tag": "EA-5601",
            "logic_no": "N/A (control loop only)",
            "trip_rows": 0,
            "row_kinds": ["alarm", "control", "mech"],
            "boilerplate": ["On any trip", "Safety PLC", "latched"],
        }
    ]
    assert R["no_sif_fixture"] == {
        "tag": "EA-5601",
        "protection": [
            "TIC-5602 control",
            "PDAH-5605 alarm > 0.7 bar",
            "PSV-5607 relief 16 barg",
        ],
        "source": "TJC-LLD-IL-EA-5601 LOGIC No: N/A (control loop only); datasheet note 1",
...
def test_hand_verified_rules():
    hv = I.hand_verified()
    assert (hv["verified_by"], hv["verified_on"]) == (
        "executor (role alias EXEC-1)",
        "2026-08-27",
    ) and len(hv["sets"]) == 8
    pngs = {os.path.basename(f) for f in CTX["files"] if P.doc_class(f) == "pid"}
    for s in hv["sets"]:
        assert s["file"] in pngs and any(
            f"Set_0{s['set']}_{s['tag']}" in f for f in CTX["files"]
        )
    ...
    assert [it["set"] for it in R["CD-2"]["items"]] == [1, 2, 3, 6, 7, 8] and R["CD-2"][
        "items"
    ][2]["ref_dwg"] is None
    assert [(it["set"], it["cited"], it["expected"]) for it in R["CD-7"]["items"]] == [
        (2, "TJC-LLD-DS-GA-1201A", "TJC-LLD-DS-YD-2301"),
        (4, "TJC-LLD-PF-GA-4501A", "TJC-LLD-DS-KC-4501"),
        (7, "TJC-LLD-DS-GA-7801A", "TJC-LLD-DS-CT-7801"),
        (8, "TJC-LLD-GA-FA-8901", "TJC-LLD-DS-FA-8901"),
    ]
    assert R["CD-8"]["items"] == [
        {"set": 2, "tag": "YD-2301", "cited": "SEQ-1201", "expected": "SEQ-5500"}
    ]
    assert [it["set"] for it in R["CD-9"]["items"]] == [2, 4, 7, 8]
    assert [
        (it["set"], it["kind"], it["sibling_confirmed"]) for it in R["CD-18"]["items"]
    ] == [
        (2, "footprint", True),
        (2, "instrument identity", True),
        (4, "revision", True),
    ]
...
def test_deterministic_and_serialisable():
    ...
    b = json.dumps(
        I.compute(dict(CTX, rows=W.load()), write_aliases=False), sort_keys=True
    )
