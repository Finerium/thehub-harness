...
CTX = {"rows": ROWS, "pops": POPS, "opl": TEXTS, "parsed": PARSED, "files": P.corpus_files()}
FX = C.compute(CTX)
T62 = {layer: {pop: {r["t"]: r for r in rows} for pop, rows in FX["coverage"][layer].items()} for layer in FX["coverage"]}
UNCOVERED_14 = ["WO-240007", "WO-240013", "WO-240033", "WO-240034", "WO-240037", "WO-240039", "WO-240063", "WO-240064",
                "WO-240089", "WO-240094", "WO-240120", "WO-240167", "WO-240169", "WO-240196"]   # = plan 5.7 list, unchanged under -raw


def test_populations_and_method():
    assert (len(POPS["failure"]), len(POPS["unplanned_failure"]), len(POPS["planned_flagged"]), len(POPS["unplanned_breakdowns"])) == (66, 57, 8, 23)
    m = FX["method"]
    assert m["stop_list_size"] == 65 and m["stop_list"] == sorted(C.STOP)
    assert m["stop_list_sha256"] == hashlib.sha256("\n".join(sorted(C.STOP)).encode("utf-8")).hexdigest()
    assert m["stop_list_sha256"].startswith("8262d5122704987d")   # observed; xcheck.py recomputed it independently
    assert m["unscoreable_ids"] == []   # every work order has a field with >= 3 content tokens (8 thermography rows score 0.0 but are scoreable)
    assert (m["t"], m["window_multiplier"], m["extractor"], m["uncovered_rule"]) == (0.62, 2, "pdftotext -raw", "score <= t")
    assert "the" in C.STOP and C.tokens("The VSHH-1201 trips at > 7.1 mm/s") == ["vshh-1201", "trips", "7.1", "mm/s"]
...
def test_strict_layer_is_composed_from_the_taught_sections():
    ...
    for k, t in TEXTS.items():
        ...
        for absent in ("COMMON PROBLEMS", "Prepared by", "Date of Sharing", "Reviewed by"):
            assert absent not in s, (k, absent)
        for taught in (PARSED[k]["purpose"], PARSED[k]["safety_text"], PARSED[k]["tools_text"],
                       PARSED[k]["steps_text"], PARSED[k]["key_learning"]):
    ...
    assert "COMMON PROBLEMS" not in leak and "Spiral-wound gasket unevenly seated" in leak
...
def test_generous_unplanned_failure_headline():
    ...
    assert (g[0.62]["n"], g[0.62]["uncovered"], g[0.62]["pct"], g[0.62]["unplanned_bd"], g[0.62]["downtime_h"], g[0.62]["cost_idr"]) == (57, 14, 24.6, 5, 74.5, 93_721_000)
    assert g[0.62]["uncovered_ids"] == UNCOVERED_14
    assert g[0.62]["by_tag"] == {"CT-7801": 2, "DC-3401A": 2, "EA-5601": 1, "FA-8901": 1, "GA-1201A": 2, "KC-4501": 2, "YD-2301": 4}
    assert [g[t]["uncovered"] for t in (0.50, 0.55, 0.60, 0.62, 0.65, 0.70)] == [13, 13, 14, 14, 14, 14]   # flat within 1 (LD-07 item 1)
    assert (g[0.75]["uncovered"], g[0.75]["unplanned_bd"], g[0.75]["downtime_h"], g[0.75]["cost_idr"]) == (20, 8, 94.5, 124_806_000)
    assert all(g[t]["unplanned_bd"] == 5 and g[t]["cost_idr"] == 93_721_000 for t in (0.50, 0.55, 0.60, 0.62, 0.65, 0.70))
...
def test_strict_unplanned_failure():
    ...
    assert (s[0.62]["n"], s[0.62]["uncovered"], s[0.62]["pct"], s[0.62]["unplanned_bd"], s[0.62]["downtime_h"], s[0.62]["cost_idr"]) == (57, 41, 71.9, 14, 146.0, 198_418_000)
    assert [s[t]["uncovered"] for t in (0.50, 0.55, 0.60, 0.62, 0.65, 0.70, 0.75)] == [32, 32, 41, 41, 41, 43, 52]
    assert [s[t]["unplanned_bd"] for t in (0.50, 0.55, 0.60, 0.62, 0.65, 0.70, 0.75)] == [12, 12, 14, 14, 14, 15, 19]
    assert [s[t]["downtime_h"] for t in (0.50, 0.55, 0.60, 0.62, 0.65, 0.70, 0.75)] == [132.0, 132.0, 146.0, 146.0, 146.0, 158.0, 182.0]
    assert [s[t]["cost_idr"] for t in (0.50, 0.55, 0.60, 0.62, 0.65, 0.70, 0.75)] == [178_671_000, 178_671_000, 198_418_000, 198_418_000, 198_418_000, 215_980_000, 260_448_000]
    assert s[0.62]["by_tag"] == {"CT-7801": 4, "DC-3401A": 7, "EA-5601": 6, "FA-8901": 3, "GA-1201A": 7, "KC-4501": 4, "LV-6701": 4, "YD-2301": 6}
    assert {"WO-240056", "WO-240062"} <= set(s[0.62]["uncovered_ids"]) and "WO-240112" not in s[0.62]["uncovered_ids"]
    assert FX["coverage_scores"]["WO-240056"]["strict"] == {"score": 0.4, "unit": ["Root_Cause", "OPL-DC-3401A-06"]}
    for t in (0.50, 0.55, 0.60, 0.62, 0.65, 0.70, 0.75):
        assert set(T62["generous"]["unplanned_failure"][t]["uncovered_ids"]) <= set(s[t]["uncovered_ids"]), t
...
def test_strict_uncovered_contains_generous_uncovered():
    ...
    higher = sorted(w for w, v in sc.items() if v["strict"]["score"] > v["generous"]["score"])
    assert higher == ["WO-240005"] and (sc["WO-240005"]["generous"]["score"], sc["WO-240005"]["strict"]["score"]) == (0.5, 0.5556)
    ...
    for pop in sorted(POPS):
        for t in C.THRESHOLDS:
            ...
            assert g <= s or (pop, t) in {("all", 0.50), ("all", 0.55)}, (pop, t, sorted(g - s))
            if not g <= s:
...
def test_other_populations():
    ...
    assert (a[0.55]["uncovered"], a[0.62]["uncovered"], a[0.75]["uncovered"], a[0.62]["pct"]) == (141, 143, 160, 67.8)
    assert (sa[0.62]["uncovered"], sa[0.62]["pct"]) == (190, 90.0)
    assert (T62["generous"]["planned_flagged"][0.62]["uncovered"], T62["generous"]["planned_flagged"][0.62]["unplanned_bd"]) == (4, 0)
    assert (T62["generous"]["unplanned_breakdowns"][0.62]["uncovered"], T62["strict"]["unplanned_breakdowns"][0.62]["uncovered"]) == (5, 14)
    assert (T62["generous"]["failure"][0.62]["uncovered"], T62["strict"]["failure"][0.62]["uncovered"]) == (19, 49)
    for layer in FX["coverage"]:
        for pop, rows in FX["coverage"][layer].items():
            assert [r["t"] for r in rows] == list(C.THRESHOLDS) and all(r["n"] == len(POPS[pop]) for r in rows)
...
def test_named_work_orders():
    ...
    assert sc["WO-240062"]["strict"] == {"score": 0.6, "unit": ["Problem_Description", "OPL-DC-3401A-07"]}
    assert sc["WO-240007"]["generous"]["score"] == sc["WO-240007"]["strict"]["score"] == 0.4286   # the coupling cluster (LD-06)
    for layer in ("generous", "strict"):
        assert all("WO-240007" in r["uncovered_ids"] for r in FX["coverage"][layer]["unplanned_failure"])
    assert sc["WO-240171"]["generous"]["score"] == 0.8 and "WO-240171" not in T62["generous"]["unplanned_failure"][0.62]["uncovered_ids"]
    assert sc["WO-240037"]["generous"]["score"] == 0.3333 and "WO-240037" in T62["generous"]["unplanned_failure"][0.62]["uncovered_ids"]   # paraphrase blind spot (LD-05)
    assert sc["WO-240094"]["generous"]["score"] == 0.6   # the stop-list edge of P5: uncovered at 0.60/0.62, covered at 0.55


def test_bands_partition_the_57():
    b = FX["coverage_bands"]["unplanned_failure"]
    assert (b["t"], b["n"], b["none"], b["table_only"], b["taught"]) == (0.62, 57, 14, 27, 16)   # plan proxy 14 / 28 / 15 (D1)
    assert "WO-240056" in b["table_only_ids"]   # P10b: it was in taught while a pasted closeout row reached the strict layer
    assert b["none_ids"] == UNCOVERED_14
    ids = b["none_ids"] + b["table_only_ids"] + b["taught_ids"]
    assert len(ids) == 57 == len(set(ids)) and sorted(ids) == W.ids(POPS["unplanned_failure"])


def test_by_work_type_and_tag():
    bw = FX["coverage_by_work_type"]
    assert bw["work_types"] == {"Calibration": {"n": 12, "uncovered": 4}, "Corrective": {"n": 53, "uncovered": 13}, "Inspection": {"n": 30, "uncovered": 26},
                                "Overhaul": {"n": 6, "uncovered": 3}, "Predictive": {"n": 42, "uncovered": 42}, "Preventive": {"n": 68, "uncovered": 55}}   # HN-V-01
    assert bw["breakdowns"] == {"n": 31, "uncovered": 9, "planned_flagged_uncovered": 4, "unplanned_uncovered": 5}
    assert sum(v["n"] for v in bw["work_types"].values()) == 211 and sum(v["uncovered"] for v in bw["work_types"].values()) == 143
    bt = FX["coverage_by_tag"]["tags"]
    assert bt == {"CT-7801": {"n_unplanned_failure": 7, "uncovered_generous": 2, "uncovered_strict": 4},
                  "DC-3401A": {"n_unplanned_failure": 7, "uncovered_generous": 2, "uncovered_strict": 7},
                  "EA-5601": {"n_unplanned_failure": 8, "uncovered_generous": 1, "uncovered_strict": 6},
                  "FA-8901": {"n_unplanned_failure": 5, "uncovered_generous": 1, "uncovered_strict": 3},
                  "GA-1201A": {"n_unplanned_failure": 7, "uncovered_generous": 2, "uncovered_strict": 7},
                  "KC-4501": {"n_unplanned_failure": 8, "uncovered_generous": 2, "uncovered_strict": 4},
                  "LV-6701": {"n_unplanned_failure": 7, "uncovered_generous": 0, "uncovered_strict": 4},
                  "YD-2301": {"n_unplanned_failure": 8, "uncovered_generous": 4, "uncovered_strict": 6}}
    assert sum(v["uncovered_generous"] for v in bt.values()) == 14 and sum(v["uncovered_strict"] for v in bt.values()) == 41
...
def test_verbatim():
    ...
    v = FX["verbatim"]
    assert v["generous"]["by_field"] == {"Corrective_Action": 42, "Problem_Description": 47, "Root_Cause": 45}
    assert v["generous"]["any"] == 47 == len(v["generous"]["any_ids"]) and v["generous"]["any_ids"] == sorted(v["generous"]["any_ids"])
    assert v["strict"]["by_field"] == {"Corrective_Action": 0, "Problem_Description": 0, "Root_Cause": 0} and v["strict"]["any_ids"] == []
    assert all(FX["coverage_scores"][w]["generous"]["score"] == 1.0 for w in v["generous"]["any_ids"])   # verbatim rows score 1.0 (LD-07 item 1)
...
def test_no_copied_troubleshooting_cell_survives_in_the_strict_layer():
    ...
    assert sum(O.PAGE_FOOTER in t for t in TEXTS.values()) == 56   # every lesson carries it exactly once, in the generous layer
...
def test_no_narrative_field_survives_verbatim_in_the_strict_layer():
    ...
    v = FX["verbatim"]
    assert v["strict"] == {"by_field": {"Corrective_Action": 0, "Problem_Description": 0, "Root_Cause": 0}, "any": 0, "any_ids": []}
    strict = O.strict_texts(PARSED)
    hits = []
    for w in ROWS:
        for cf in W.NARR:
    ...
    assert len(rc) >= C.VERBATIM_MIN_CHARS and rc in P.canonical(TEXTS["OPL-DC-3401A-07"])
    assert all(rc not in t for t in strict.values()) and "WO-240062" in v["generous"]["any_ids"]
...
def test_generous_layer_unchanged_by_the_strict_re_freeze():
    ...
    g = T62["generous"]["unplanned_failure"]
    assert (g[0.62]["uncovered"], g[0.62]["unplanned_bd"], g[0.62]["downtime_h"], g[0.62]["cost_idr"]) == (14, 5, 74.5, 93_721_000)
    assert g[0.62]["uncovered_ids"] == UNCOVERED_14
    assert C.score(ROWS, C.lessons_by_tag(TEXTS)) == {w: v["generous"] for w, v in FX["coverage_scores"].items()}
    assert FX["verbatim"]["generous"]["by_field"] == {"Corrective_Action": 42, "Problem_Description": 47, "Root_Cause": 45}
    b = FX["coverage_bands"]["unplanned_failure"]
    assert (b["none"], b["table_only"], b["taught"]) == (14, 27, 16)   # only the strict side moves: WO-240056 none -> table_only
...
def test_agreement_synthetic_labels():
    ...
    assert a == {"n": 2, "precision": None, "recall": 0.0, "tp": 0, "fp": 0, "fn": 1, "tn": 1}
    lab = C.labelled_table(ROWS, labels)
    w7 = next(r for r in ROWS if r["WO_Number"] == "WO-240007")
    assert (lab["n"], lab["uncovered"], lab["uncovered_ids"], lab["unplanned_bd"]) == (2, 1, ["WO-240007"], 1)
    assert (lab["downtime_h"], lab["cost_idr"]) == (w7["Downtime_Hours"], w7["Total_Cost_IDR"])
...
def test_labels_status_never_both_absent():
    ...
    if FX["labels_agreement"] is None:
        ...
        assert FX["method"]["labels_status"] == (C.DRAFT_STATUS if FX["labelled_draft"] else "pending (proxy numbers in use)")
    else:
        assert {"generous", "strict", "t"} <= set(FX["labels_agreement"]) and FX["labelled"]["n"] == FX["labels_agreement"]["generous"]["n"]
...
def test_draft_labels_agreement():
    ...
    assert a["t"] == 0.62
    assert {k: a["generous"][k] for k in ("n", "tp", "fp", "fn", "tn")} == {"n": 57, "tp": 42, "fp": 1, "fn": 5, "tn": 9}
    assert (a["generous"]["precision"], a["generous"]["recall"]) == (round(42 / 43, 4), round(42 / 47, 4))
    assert {k: a["strict"][k] for k in ("n", "tp", "fp", "fn", "tn")} == {"n": 57, "tp": 15, "fp": 1, "fn": 32, "tn": 9}
    assert (a["strict"]["precision"], a["strict"]["recall"]) == (round(15 / 16, 4), round(15 / 47, 4))
...
def test_legacy_window_reproduces_v10_headline():
    ...
    assert 170_000_000 <= r[0.62]["cost_idr_all"] <= 185_000_000 and r[0.62]["cost_idr_all"] == 178_870_000
    assert r[0.62]["downtime_h_all"] == 238.5
    assert (r[0.62]["unplanned_bd"], r[0.62]["downtime_h"], r[0.62]["cost_idr"]) == (8, 108.5, 124_716_000)
    assert [r[t]["uncovered"] for t in C.THRESHOLDS] == [101, 141, 153, 158, 159, 162, 163]   # the band collapses at 0.50 (LD-01 noise floor)


def test_fixture_is_json_and_keys():
    assert set(FX) == {"method", "coverage", "coverage_scores", "coverage_bands", "coverage_by_work_type", "coverage_by_tag", "verbatim",
                       "labels_agreement", "labels_agreement_draft", "labelled", "labelled_draft", "legacy"}
    assert set(FX["coverage"]) == {"generous", "strict"} and set(FX["coverage"]["generous"]) == set(POPS)
