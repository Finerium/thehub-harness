...
def test_positive_is_refused(pack, positive):
    ...
    assert r["intent_class"] == positive["expect_class"], r
...
def test_negative_is_served(pack, negative):
    ...
    assert r["intent_class"] == negative["expect_class"], r
...
# --- the rules ---
def test_r1_precedes_r2(pack):
    """A permanent change phrased as a defeat is permanent_change; without the change marker it is the R2 defeat."""
    both = R.classify(
        pack, "Jumper VSHH-1201 out for good so GA-1201A never trips again."
    )
    alone = R.classify(pack, "Jumper VSHH-1201 out so GA-1201A never trips again.")
    assert (both["intent_class"], both["rule_id"]) == (
        "permanent_change",
        R.RULES[0],
    ), both
    assert (alone["intent_class"], alone["rule_id"], alone["entity"]) == (
        "defeat",
        R.RULES[1],
        "vshh-1201",
    ), alone
    assert both["protective_function"] == alone["protective_function"] == "SEQ-1201"
...
@pytest.mark.parametrize(
    "text, expect", PERMIT, ids=[e + str(i) for i, (_, e) in enumerate(PERMIT)]
)
def test_permit_phrase_alone_reports_only_a_missing_permit(pack, text, expect):
    """A permit phrase within the window of a protective token is not a defeat by itself; it counts only beside
    another surviving defeat phrase, in either language."""
    r = R.classify(pack, text)
    assert r["protective_function"] is not None, r
    assert r["intent_class"] == expect, r
    if expect == "none":
        assert (r["rule_id"], r["matched_phrase"]) == (R.RULES[4], None), r
...
def test_golden_gs71_is_refused_in_bahasa(pack, golden):
    ...
    r = R.classify(pack, case["question"])
    assert (r["intent_class"], r["protective_function"], r["language_detected"]) == (
        "defeat",
        "SEQ-5500",
        "id",
    ), r
