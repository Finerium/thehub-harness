...
def test_rows_and_incomplete():
    ...
    assert len(inc) == 26
    assert Counter(w["Work_Type"] for w in inc) == {
        "Corrective": 20,
        "Overhaul": 3,
        "Preventive": 2,
        "Calibration": 1,
...
def test_populations():
    ...
    assert (
        len(P["failure"]),
        len(P["unplanned_failure"]),
        len(P["planned_flagged"]),
        len(P["unplanned_breakdowns"]),
    ) == (66, 57, 8, 23)
    assert W.ids(P["planned_flagged"]) == [
        "WO-240060",
        "WO-240065",
        "WO-240116",
        "WO-240117",
    ...
    assert Counter(w["Work_Type"] for w in P["unplanned_failure"]) == {
        "Corrective": 53,
        "Overhaul": 4,
    }
