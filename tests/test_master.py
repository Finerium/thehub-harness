...
def test_equipment_master_reconciles_to_workbook(out):
    ...
    assert {t: by[t]["sil_sheet"] for t in TAGS} == {
        "GA-1201A": 1,
        "YD-2301": 2,
        "DC-3401A": 2,
        "KC-4501": 2,
    ...
    assert (
        by["YD-2301"]["interlock_sheet"]
        == by["YD-2301"]["interlock_workbook"]
        == "SEQ-5500"
    )
    assert (
        by["EA-5601"]["interlock_sheet"] is None
        and by["EA-5601"]["interlock_workbook"] is None
    )
    assert (
        by["CT-7801"]["criticality_datasheet"] == "NON CRITICAL"
        and by["CT-7801"]["unplanned_breakdown_h"] == 46.0
    )
    assert (
        by["EA-5601"]["criticality_datasheet"] == "LOW CRITICAL"
        and by["EA-5601"]["unplanned_breakdown_h"] == 54.0
    )
    for e in (
        em
    ):  # workbook and datasheet agree on area and criticality for all eight assets
        assert (
            e["area_workbook"] == e["area_datasheet"]
            and e["criticality_workbook"] == e["criticality_datasheet"]
        )
...
def test_interlock_rows_typed(out):
    ...
    assert [
        ...
        for r in il["EA-5601"]["rows"]
    ] == [
        ("C1", "control", "TIC-5602", None, "control", ["EFF-1"]),
        ("A1", "alarm", "PDAH-5605", None, "alarm", ["EFF-2"]),
        ("R1", "mech", "PSV-5607", None, "mech", ["EFF-3"]),
    ]
    assert all(
        r["voting"] == r["vote_cell_text"]
        for t in TAGS
        for r in il[t]["rows"]
        if r["row_kind"] == "trip"
    )
    assert (
        il["EA-5601"]["header"]["logic_no"] is None
        and il["EA-5601"]["header"]["sil"] is None
    )
    assert il["EA-5601"]["trip_boilerplate"] and not any(
        r["row_kind"] == "trip" for r in il["EA-5601"]["rows"]
    )  # CD-17
    for t in TAGS:
        h = il[t]["header"]
        assert (h["work_no"], h["rev"], h["doc_no"]) == (
            "BA-0256",
            3,
            f"TJC-LLD-IL-{t}",
        )
        ...
        alarm = [
            e["id"]
            for e in il[t]["effects"]
            if e["action"] == "ANNUNCIATE ALARM ON DCS"
        ]
        for r in il[t]["rows"]:
            assert r["effects_basis"] == "H+M" and len(r["effects"]) == r["x_count"]
            assert (
                not alarm or alarm[0] in r["effects"]
            )  # every trip annunciates on the DCS (sheet note 2)
    assert sum(len(il[t]["rows"]) for t in TAGS) == 39
    assert (
        sum(1 for t in TAGS for r in il[t]["rows"] if r["effects_basis"] == "H+M") == 39
    )
...
def test_cause_and_effect_columns(out):
    ...
    assert [len(out["interlock_rows"][t]["rows"]) for t in order] == [
        6,
        6,
        6,
        6,
    ...
    rows = {
        r["id"]: r for r in by_seq["SEQ-1201"]["rows"]
    }  # DP-31: min-flow FV-1201 does not open on a vibration trip
    assert (rows["T3"]["tag"], rows["T3"]["setpoint_text"], rows["T3"]["voting"]) == (
        "VSHH-1201",
        "> 7.1 mm/s RMS",
        "1oo2",
    )
    assert (
        rows["T3"]["effects"] == ["EFF-1", "EFF-2", "EFF-4", "EFF-5"]
        and "EFF-3" not in rows["T3"]["effects"]
    )
    assert (
        next(e for e in by_seq["SEQ-1201"]["effects"] if e["id"] == "EFF-3")[
            "final_element"
        ]
        == "OPEN MIN-FLOW FV-1201"
    )

    seq5500 = by_seq["SEQ-5500"]
    assert seq5500["tag"] == "YD-2301" and len(seq5500["rows"]) == 6
    assert [r["tag"] for r in seq5500["rows"]] == [
        "TSHH-2301",
        "FSLL-2302",
        "LSHH-2303",
        "SSLL-2305",
    ...
    t2 = {r["id"]: r for r in by_seq["SEQ-3401"]["rows"]}[
        "T2"
    ]  # observed: EFF-5 (the DCS alarm) as well
    assert t2["tag"] == "PSHH-3404" and {"EFF-2", "EFF-3"} <= set(t2["effects"])
    assert t2["effects"] == ["EFF-2", "EFF-3", "EFF-5"]

    ea = by_seq["EA-5601"]
    assert (
        ea["kind"] == "control_loop_only"
        and ea["header"]["sil"] is None
        and ea["header"]["logic_no"] is None
    )
    assert [(r["id"], r["row_kind"]) for r in ea["rows"]] == [
        ("C1", "control"),
        ("A1", "alarm"),
        ("R1", "mech"),
    ]
    ...
    for seq, sheet in sorted(by_seq.items()):
        n = len(sheet["effects"])
        assert [e["id"] for e in sheet["effects"]] == [
            f"EFF-{i}" for i in range(1, n + 1)
        ], seq
        assert all(e["final_element"] == e["action"] for e in sheet["effects"]), seq
        for r in sheet["rows"]:
            assert r["effects"] == sorted(
                set(r["effects"]), key=lambda e: int(e.split("-")[1])
            ), (seq, r["id"])
            assert set(r["effects"]) <= {e["id"] for e in sheet["effects"]}, (
                seq,
                r["id"],
            )
...
def test_hand_effects_cannot_contradict_the_raw_text():
    ...
    assert (
        master.parse_interlock("GA-1201A", text, hand)["rows"][0]["setpoint_text"]
        == "< 0.5 barg"
    )
...
def test_proof_tests_counts(out):
    ...
    pt = out["proof_tests"]
    assert {k: v["count"] for k, v in pt["classes"].items()} == {
        "sis_proof_test": 18,
    ...
    assert pt["total"] == 33
    assert "WO-240012" in [
        i["wo"] for i in pt["classes"]["sil_logic_proof_test"]["items"]
    ]
    assert sorted(i["wo"] for i in pt["classes"]["psv_statutory_test"]["items"]) == [
        "WO-240060",
        "WO-240116",
        "WO-240189",
    ]
    ...
    assert pt["last_by_asset"]["EA-5601"] == {
        "sis_proof_test": None,
        "sil_logic_proof_test": None,
        "calibration_proof_test": None,
        "psv_statutory_test": "2024-07-31",
    }
...
def test_personnel_roles(out):
    ...
    assert pe["people"]["EMP-0901"] == {
        "name": "Zulkarnain Hasan",
        "role_class": "manager",
        "roles": {"opl_approver": 28},
        "sources": ["opl"],
    }
    assert [a["wo"] for a in pe["anomalies"]] == ["WO-240142", "WO-240161"]
    assert pe["anomalies"][0]["flags"] == [
        "Executed_By id class is reporter, expected executor"
    ]
    assert (
        pe["anomalies"][1]["ids"]["Reported_By"]
        == pe["anomalies"][1]["ids"]["Executed_By"]
        == "EMP-2010"
    )
...
def test_families_membership_and_r(out):
    ...
    assert m["gasket seating and manway leaks"] == [
        "WO-240062",
        "WO-240111",
        "WO-240145",
        "WO-240195",
    ]
    assert set(m["PTFE packing and stem friction"]) >= {
        "WO-240138",
        "WO-240135",
        "WO-240029",
        "WO-240028",
    }
    assert set(m["exchanger fouling and thermal performance"]) >= {
        "WO-240110",
        "WO-240119",
        "WO-240089",
    }
    ...
    assert set(m["alignment and bearing degradation"]) >= {
        "WO-240003",
        "WO-240004",
        "WO-240007",
        "WO-240088",
    ...
    assert m["lubrication supply and oil-system degradation"] == [
        "WO-240084",
        "WO-240091",
    ]
    assert {t: v["r"] for t, v in fa["r_by_tag"].items()} == {
        "CT-7801": 0.4286,
        "DC-3401A": 0.1429,
        "EA-5601": 0.25,
        "FA-8901": 0.2,
    ...
    assert [f["members_outside_unplanned57"] for f in fa["families"]] == [
        [],
        [],
        ["WO-240119"],
        [],
...
def test_chains_ga1201a(out):
    ...
    assert {
        ("WO-240003", "WO-240004", "misalignment"),
        ("WO-240004", "WO-240007", "misalignment"),
        ("WO-240003", "WO-240007", "misalignment"),
    } <= trip
    assert not any(
        l["to_wo"] == "WO-240002" for l in ch["links"]
    )  # no shared mechanism noun reaches the seal failure
    l = next(
        l
        for l in ch["links"]
        if (l["from_wo"], l["to_wo"]) == ("WO-240003", "WO-240004")
    )
    assert (l["days"], l["field"], l["linking_sentence"]) == (
        33,
        "Root_Cause",
        "Outer race spalling on DE bearing due to prolonged misalignment",
    )
    assert all(0 < l["days"] <= 365 for l in ch["links"])
    assert len(ch["lexicon"]) == 45 and {
        "misalignment",
        "spalling",
        "fouling",
        "plugged",
    ...
    with open(os.path.join(PACKAGES, "chains.json"), encoding="utf-8") as f:
        assert (
            json.load(f) == ch
        )  # the reviewed package artefact is the harness output (regenerate with `make packages`)
...
def test_datasheet_spot_verified(out):
    ...
    assert v[("CT-7801", "gearbox_oil_grade")] == (220, "ISO VG") and v[
        ("CT-7801", "gearbox_oil_volume")
    ] == (4.5, "L")
    assert v[("EA-5601", "tube_design_pressure")] == (16, "barg") and v[
        ("EA-5601", "tube_design_temperature")
    ] == (200, "degC")
...
def test_revision_spot_matches_package(out):
    ...
    assert sorted(rs) == TAGS and all(
        sorted(rs[t]) == ["datasheet", "ga_drawing", "interlock", "plot_plan"]
        for t in TAGS
    )
    g = rs["GA-1201A"]
    assert g["datasheet"] == {
        "doc_no": "TJC-LLD-DS-GA-1201A",
        "rev": 3,
    ...
    assert (g["ga_drawing"]["rev"], g["ga_drawing"]["status"]) == (
        "0",
        "ISSUED FOR CONSTRUCTION",
    )
    assert [
        (h["rev"], h["description"], h["date"]) for h in g["plot_plan"]["history"]
    ] == [
        ("A", "ISSUED FOR REVIEW", "2026-05-15"),
        ("B", "ISSUED FOR APPROVAL", "2026-05-27"),
        ("0", "ISSUED FOR CONSTRUCTION", "2026-06-05"),
    ]
    for t in TAGS:
...
def test_compute_is_order_independent_and_serialisable(ctx, out):
    ...
    shuffled = dict(
        ctx,
        rows=rnd.sample(ctx["rows"], len(ctx["rows"])),
