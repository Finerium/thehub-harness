...
DRAFT_LABELS_PATH = os.path.join(
    PACKAGES, "coverage_labels.draft.json"
)  # WS1 Task 1.6: the machine-drafted set
DRAFT_STATUS = (
    "machine-drafted labels present (packages/coverage_labels.draft.json); human adjudication pending (OQ-6); "
...
def tokens(s):
    ...
    return [
        t
        for t in re.findall(r"[a-z0-9]+(?:[-./][a-z0-9]+)*", s.lower())
...
def contain_doc(ftoks, dtoks, mult=2):
    ...
    n = len(ftoks) * mult
    if len(dtoks) <= n:
        return len(F & set(dtoks)) / len(F)
    best = 0
    win = Counter(dtoks[:n])
    best = sum(1 for t in F if t in win)
        ...
        rows.append(
            {
                ...
                "n": len(pop),
                "uncovered": len(unc),
                "pct": round(100 * len(unc) / len(pop), 1) if pop else 0.0,
                "unplanned_bd": len(ub),
                ...
                "by_tag": dict(
                    sorted(Counter(w["Equipment_Tag"] for w in unc).items())
                ),
            }
        )
    return rows
...
def verbatim(rows, texts):
    ...
    by_tag: dict[str, str] = {}
    for oid in sorted(texts):
        by_tag[tag_of_opl(oid)] = (
            by_tag.get(tag_of_opl(oid), "") + " " + canonical(texts[oid])
        )
...
def kappa(records):
    ...
    taught = [
        tuple(bool(x["covered_by"]) and not x["table_only"] for x in r["labellers"])
        for r in rs
    ]
    return {
        "n": len(rs),
        "labellers": [x["labeller"] for x in rs[0]["labellers"]],
        "covered": _kappa(cov),
        "taught": _kappa(taught),
...
def labelled_table(rows, labels, t=T):
    ...
    pseudo = {
        w["WO_Number"]: {"score": 1.0 if labels[w["WO_Number"]] else 0.0} for w in pop
    }
...
def legacy_corpus(texts):
    ...
    for oid in sorted(texts):
        out[tag_of_opl(oid)] = (
            out.get(tag_of_opl(oid), "")
            + " "
            + re.sub(r"\s+", " ", canonical(texts[oid]).lower())
        )
    return out
...
def legacy_score(rows, corpus_by_tag):
    ...
    out = {}
    for w in rows:
        corpus = corpus_by_tag.get(w["Equipment_Tag"], "")
        b = 0.0
        for cf in NARR:
            v = str(w[cf] or "")
            if len(v) >= 20:
...
def compute(ctx):
    ...
    t = ctx.get(
        "t", T
    )  # the CLI's --t (analyze_corpus); the threshold rows always include it
    ...
    coverage = {
        name: {p: table(pops[p], sc, thresholds) for p in sorted(pops)}
    ...
    taught = sorted(
        ...
        if w["WO_Number"] not in set(g_unc) | set(s_unc)
    )
    bands = {
        "t": t,
        "n": len(pops["unplanned_failure"]),
        "none": len(none),
        "table_only": len(table_only),
        "taught": len(taught),
        "none_ids": none,
        "table_only_ids": table_only,
        "taught_ids": taught,
    }
    g = scores["generous"]
    by_type = {
        wt: {"n": 0, "uncovered": 0} for wt in sorted({w["Work_Type"] for w in rows})
    }
    bd = {
        "n": 0,
        "uncovered": 0,
        "planned_flagged_uncovered": 0,
        "unplanned_uncovered": 0,
    }
    for w in rows:
        unc = g[w["WO_Number"]]["score"] <= t
        by_type[w["Work_Type"]]["n"] += 1
        by_type[w["Work_Type"]]["uncovered"] += unc
        if w["breakdown_kind"]:
            bd["n"] += 1
            bd["uncovered"] += unc
            bd[w["breakdown_kind"] + "_uncovered"] += unc
    by_tag: dict[str, dict[str, Any]] = {}
    for w in pops["unplanned_failure"]:
        d = by_tag.setdefault(
            w["Equipment_Tag"],
            {"n_unplanned_failure": 0, "uncovered_generous": 0, "uncovered_strict": 0},
        )
    ...
    legacy_rows = (
        table(pops["all"], legacy_score(rows, legacy_corpus(texts)), thresholds)
        if ctx.get("legacy_window", True)
        else []
    )
    for r in legacy_rows:  # v1.0 counted every Breakdown = Yes row, planned ones included (LD-04); kept for the P6 comparison
        ub = [
            w
            for w in rows
            if w["breakdown_kind"] and w["WO_Number"] in set(r["uncovered_ids"])
        ]
        r.update(
            breakdowns_all=len(ub),
            downtime_h_all=sum(w["Downtime_Hours"] or 0 for w in ub),
    ...
    kap = kappa(
        load_label_records() if labels else load_label_records(DRAFT_LABELS_PATH)
    )
    stop = sorted(STOP)
    method = {
        ...
        "canonical": "NFKC, soft hyphens joined, whitespace collapsed",
        ...
        "labels_status": f"labelled ({len(labels)} work orders)"
        if labels
        else DRAFT_STATUS
        if draft
        else "pending (proxy numbers in use)",
    }
    return {
        "method": method,
        "coverage": coverage,
        "coverage_scores": {
            wo: {name: scores[name][wo] for name in sorted(scores)} for wo in sorted(g)
        },
        ...
        "verbatim": {
            "rule": f"canonical field of >= {VERBATIM_MIN_CHARS} chars is an exact substring of the same-asset lessons",
            **{name: verbatim(rows, t) for name, t in layers.items()},
        },
        "labels_agreement": {
            "t": t,
            "kappa": kap,
            **{name: agreement(sc, labels, t) for name, sc in scores.items()},
        }
        if labels
        else None,
        ...
        "labels_agreement_draft": {
            "t": t,
            "kappa": kap,
            **{name: agreement(sc, draft, t) for name, sc in scores.items()},
        }
        if draft
        else None,
