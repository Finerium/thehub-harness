...
IL_ROW = re.compile(
    r"(?:^| )(?P<id>[TCAR]\d) (?P<initiator>.+?) (?P<tag>"
    + INSTR
    + r") (?P<setpoint>.+?) "
...
IL_GATE = re.compile(
    r"=> ALL permissives (?P<state>TRUE) \((?P<gate>AND)\) => START (?P<tag>\S+) (?P<result>ENABLED RUN)"
)
IL_PERM = re.compile(
    r"(?:^| )(?P<n>\d) (?P<condition>.+?) (?P<signal>"
    + INSTR
    + r"|DCS reset|FSL upstream|LG gearbox|lockout|DVC6200)(?= \d |$)"
)
SETPOINT = re.compile(
    r"(?P<cmp>[<>])?\s*(?P<num>-?\d+(?:\.\d+)?)\s*(?P<unit>[A-Za-z%][A-Za-z0-9/%]*)?"
)
...
def _agree(what, got, want):
    ...
    if got != want:
        raise ValueError(
            f"interlock_effects.json disagrees with the raw text at {what}: text {got!r}, file {want!r}"
        )
...
def parse_interlock(tag, text, hand=None):
    ...
    header = {
        ...
        "work_no": h["work_no"],
        "rev": int(h["rev"]),
        "logic_no": None if h["logic_no"].startswith("N/A") else h["logic_no"],
        "logic_no_text": h["logic_no"],
    ...
    eff_seg = text[
        text.index("EFFECT ID FINAL ELEMENT / ACTION ")
        + len("EFFECT ID FINAL ELEMENT / ACTION ") : text.index(" # START PERMISSIVE")
    ]
    effects = [{"id": i, "action": a} for i, a in IL_EFF.findall(eff_seg)]
    _agree(
        f"{seq} effects",
        [(e["id"], e["action"]) for e in effects],
        [(e["id"], e["final_element"]) for e in sheet["effects"]],
    )
    effects = [
        dict(e, final_element=f["final_element"])
        for e, f in zip(effects, sheet["effects"], strict=True)
    ]

    matrix = text[
        text.index("SET POINT VOTE ") + len("SET POINT VOTE ") : text.index(" Legend:")
    ]
    n_cols = len(
        re.findall(
            r"EFF-\d+ ",
            matrix[
                : must_match(IL_ROW.search(matrix), f"matrix rows of {seq}").start()
            ],
        )
    )
    ...
    for m, hr in zip(parsed, sheet["rows"], strict=True):
        ...
        _agree(
            f"{seq} {d['id']}",
            (d["initiator"], d["tag"], d["setpoint"], val, unit, d["vote"]),
            (
                hr["initiator"],
                hr["tag"],
                hr["setpoint_text"],
        ...
        _agree(
            f"{seq} {d['id']} x_count", n_x, len(hr["effects"])
        )  # the X count is the text's own check on the hand columns
        rows.append(
            {
                "id": d["id"],
                "row_kind": ROW_KIND[d["id"][0]],
                "initiator": d["initiator"],
                "tag": d["tag"],
                "setpoint_text": d["setpoint"],
                "setpoint_value": val,
                "setpoint_unit": unit,
                ...
                "voting": d["vote"] if ROW_KIND[d["id"][0]] == "trip" else None,
                "vote_cell_text": d["vote"],
                "x_count": n_x,
                "effects": list(hr["effects"]),
                "effects_basis": "H+M",
            }
        )
    perm_seg = text[
        text.index("# START PERMISSIVE (AND-gate) SIGNAL ")
        + len("# START PERMISSIVE (AND-gate) SIGNAL ") : text.index(
            " => ALL permissives"
        )
    ]
    permissives = [
        {"n": int(p["n"]), "condition": p["condition"], "signal": p["signal"]}
        for p in IL_PERM.finditer(perm_seg)
    ]
    ...
    return {
        "tag": tag,
        "seq": seq,
        "kind": sheet.get("kind", "trip_logic"),
        "header": header,
        "effects": effects,
        "n_effect_columns": n_cols,
        "rows": rows,
        "start_permissives": permissives,
        "start_permissives_gate": gate.group("gate"),
        "start_permissives_gate_line": gate.group(0),
        "start_permissives_rule": f"ALL {len(permissives)} permissives must be TRUE (AND) before {tag} start is ENABLED",
        "notes": sheet["notes"],
        "training_values_note": "Trip set points are" in text
        and "training values" in text,
...
DS_AREA = re.compile(
    r"AREA (?P<area>\d{4} - .+?) FUNCTIONAL LOC\. (?P<floc>\S+) CRITICALITY (?P<crit>HIGH CRITICAL|LOW CRITICAL|NON CRITICAL)"
)
DS_STATUS = re.compile(
    r"DATASHEET REV Rev (?P<rev>\d+) - (?P<status>ISSUED FOR [A-Z]+)"
)
DWG = re.compile(r"DWG No\. (?P<dwg_no>TJC-LLD-(?:GA|PP)-\S+) REV:(?P<rev>\S+) SHEET")
HIST_ROW = re.compile(
    r"(?P<rev>[A-Z0-9]) (?P<desc>ISSUEDFOR[A-Z]+) (?P<by>[A-Z]{2}) (?P<date>\d{2}-\d{2}-\d{4})"
)
...
def parse_drawing(text):
    ...
    hist = [
        {
            "rev": r,
            "description": desc.replace("ISSUEDFOR", "ISSUED FOR "),
            "by": by,
    ...
    return {
        "dwg_no": d["dwg_no"],
        "rev": d["rev"],
        "status": cur[0]["description"] if cur else None,
        "history": hist,
    }
...
def revision_spot(datasheets, drawings, plots, interlocks):
    ...
    for tag in sorted(datasheets):
        ...
        out[tag] = {
            ...
            "ga_drawing": drawings[tag],
            "plot_plan": plots[tag],
            "interlock": {
                k: il[k] for k in ("doc_no", "work_no", "rev", "logic_no", "sil")
            },
        }
    return out
        ...
        out.append(
            {
                ...
                "area_workbook": _majority(
                    f"{r['Area_Code']} - {r['Area_Name']}" for r in rs
                ),
                ...
                "planned_flagged_rows": sum(
                    1 for r in rs if r["breakdown_kind"] == "planned_flagged"
                ),
                "unplanned_breakdowns": len(ub),
                "unplanned_breakdown_h": sum(r["Downtime_Hours"] or 0.0 for r in ub),
                "unplanned_failure_rows": sum(
                    1 for r in rs if r["is_failure"] and not r["is_planned"]
                ),
    ...
    for name, wt, rx in PROOF_CLASSES:
        hits = sorted(
            (
                ...
                for r in rows
                if r["Work_Type"] == wt
                and rx.search(str(r["Problem_Description"] or ""))
            ),
            ...
            "items": [
                {
                    ...
                    "seq": None
                    if r["Related_Interlock"] in (None, "", "-")
                    else r["Related_Interlock"],
...
WO_ROLES = (
    ("Reported_By", "wo_reporter", "reporter"),
    ("Executed_By", "wo_executor", "executor"),
    ("Approved_By", "wo_approver", "supervisor"),
)
...
def personnel(rows, parsed):
    ...
    for r in sorted(rows, key=lambda r: r["WO_Number"]):
        ...
        for col, role, expect in WO_ROLES:
            ...
            if role_class(m["id"]) != expect:
                flags.append(
                    f"{col} id class is {role_class(m['id'])}, expected {expect}"
                )
        ...
        if flags:
            anomalies.append(
                {"wo": r["WO_Number"], "ids": dict(sorted(ids.items())), "flags": flags}
            )
    for oid in sorted(parsed):
        lp = parsed[oid]
        for key, role in (
            ("reviewed_by", "opl_reviewer"),
            ("approved_by", "opl_approver"),
        ):
            eid = lp[key + "_id"]
            if not eid:
                continue
    ...
    table = {
        eid: {
            ...
            "roles": dict(sorted(p["roles"].items())),
            "sources": sorted(p["sources"]),
        }
        for eid, p in sorted(people.items())
    }
    return {
        "people": table,
        "anomalies": anomalies,
        "opl_managers_absent_from_workbook": sorted(
            e
            for e, p in table.items()
            if p["role_class"] == "manager" and "workbook" not in p["sources"]
        ),
        "opl_reviewers_are_wo_approvers": sorted(
            e
            for e, p in table.items()
            if "opl_reviewer" in p["roles"] and "wo_approver" in p["roles"]
        ),
        "prepared_by_is_role": sorted(
            {v["prepared_by_role"] for v in parsed.values() if v["prepared_by_role"]}
        ),
    }
...
def families(rows, pops):
    ...
    for tag in sorted({r["Equipment_Tag"] for r in rows}):
        ...
        r_by_tag[tag] = {
            "unplanned_failure_rows": len(n),
            "in_family": len(k),
            "member_wos": k,
            "r": round(len(k) / len(n), 4) if n else 0.0,
        }
    return {
        "families": [
            {
                "family": f["family"],
                "basis": f["basis"],
                "member_wos": f["member_wos"],
                "member_basis": f["member_basis"],
                "tags": sorted({tag_of[w] for w in f["member_wos"] if w in tag_of}),
                "members_outside_unplanned57": sorted(
                    w for w in f["member_wos"] if w not in uf
                ),
            }
            for f in fam
        ],
...
def chains(pops):
    ...
    fail = sorted(
        pops["failure"],
        key=lambda r: (r["Equipment_Tag"], r["Report_Date"], r["WO_Number"]),
    )
    for a in fail:
        ...
        for b in fail:
            ...
            if b["Equipment_Tag"] != a["Equipment_Tag"] or not (
                0 < days <= CHAIN_WINDOW_DAYS
            ):
                continue
            for field in ("Root_Cause", "Problem_Description"):
                for noun in sorted(na & _nouns(b[field])):
                    if not any(
                        l["from_wo"] == a["WO_Number"]
                        and l["to_wo"] == b["WO_Number"]
                        and l["noun"] == noun
                        for l in links
                    ):
                        links.append(
    ...
    return {
        "lexicon": LEXICON,
        "population": "failure (66)",
        "window_days": CHAIN_WINDOW_DAYS,
        "links": links,
        "total": len(links),
        "by_tag": dict(sorted(Counter(l["tag"] for l in links).items())),
    }
...
def datasheet_spot(datasheet_texts):
    ...
    for e in sorted(spot, key=lambda e: (e["tag"], e["field"])):
        out.append(
            dict(
                e,
                verified=e["quote"] in datasheet_texts[e["tag"]]
                and e["value_text"] in e["quote"],
            )
        )
    return {
        "entries": out,
        "n": len(out),
        "verified": sum(1 for e in out if e["verified"]),
...
def compute(ctx):
    ...
    hand = hand_effects()
    interlocks = {
        t: parse_interlock(t, x, hand) for t, x in class_texts("interlock").items()
    }
...
def write_packages():
    ...
    with open(out, "w", encoding="utf-8") as f:
        json.dump(
            chains(context()["pops"]), f, sort_keys=True, indent=1, ensure_ascii=False
        )
        f.write("\n")
    return out
